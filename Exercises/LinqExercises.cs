...
public sealed class LinqExercises
{
    ...
    {
        return UniversityData.Students.Select(e => $"{e.Email}").ToList();
    }
    ...
    {
        return UniversityData.Students.OrderBy(student => student.LastName).OrderBy(student => student.FirstName)
            .Select(student => $"{student.IndexNumber} {student.FirstName} {student.FirstName} {student.LastName}")
            .ToList();
    }
    ...
    {
        return [UniversityData.Courses.Where(s => s.Category == "Analytics").Select(c => $"{c.Title} {c.StartDate}").FirstOrDefault() ?? "ALALL"];
    }
    ...
    {
        return UniversityData.Courses.Skip(2).Take(2).Select(e=>$"{e.Title} {e.Category}");
    }
    ...
    {
        return UniversityData.Students.Join(UniversityData.Enrollments, st => st.Id, z => z.StudentId,
            (st, e) => $"{st.FirstName} {st.LastName} - {e.CourseId}").ToList();
    }
    ...
    {
        return UniversityData.Enrollments.Join(UniversityData.Students,e => e.StudentId, s => s.Id,(e,s) => new {e.CourseId,s.FirstName,s.LastName})
            .Join(UniversityData.Courses, mix => mix.CourseId, c => c.Id,(mix,c)=>$"{mix.FirstName} {mix.LastName} - {c.Title}").ToList();
    }
    ...
    {
        return UniversityData.Enrollments.GroupBy(e => e.CourseId).Join(UniversityData.Courses, gr => gr.Key,c => c.Id,(gr, c) => $"{c.Title} {gr.Count()}").ToList();
    }
    ...
    {
        return UniversityData.Enrollments.Where(e=> e.FinalGrade is not null).GroupBy(e => e.CourseId).Join(UniversityData.Courses,g=>g.Key,c=>c.Id,(g,c)=>$"{c.Title}:{g.Average(e => e.FinalGrade)}").ToList();
    }
    ...
    {
        return UniversityData.Lecturers.GroupJoin(UniversityData.Courses, l => l.Id,c => c.LecturerId,(l, courseGroup) => $"{l.FirstName} {l.LastName}: {courseGroup.Count()}").ToList();
    }
    ...
    {
        return UniversityData.Enrollments.Where(e=> e.FinalGrade is not null).GroupBy(e => e.StudentId).Join(UniversityData.Students,gr => gr.Key,s=>s.Id,(gr,s)=>$"{s.FirstName} {s.LastName}: {gr.Max(e => e.FinalGrade)}").ToList();
    }
    ...
    {
        return UniversityData.Enrollments.Where(e => e.IsActive).GroupBy(e => e.StudentId).Where(g => g.Count() > 1).Join(UniversityData.Students, g => g.Key, s => s.Id, (g, s) => $"{s.FirstName} {s.LastName} - {g.Count()}").ToList();
    }
    ...
    {
        return UniversityData.Courses.Where(c => c.StartDate.Month == 4 && c.StartDate.Year == 2026).Join(UniversityData.Enrollments, c => c.Id, e => e.CourseId, (c, e) => new { c.Title, e.FinalGrade }).GroupBy(x => x.Title).Where(g => g.All(x => x.FinalGrade == null)).Select(g => g.Key).ToList();
    }
